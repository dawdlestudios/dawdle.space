...
export const SitesList = () => {
	const { data, isLoading, error } = useQuery({
		queryKey: ["sites"],
		queryFn: () => api["/api/public/sites"].get().json(),
	});

	// const users = data?.filter(
	// 	(site) => site.type === "user" && site.username !== "test",
	// );
					...
					<ul>
						{users?.map((user) => (
							<li key={user.username}>
								{">>"}{" "}
								<a href={`https://${user.username}.dawdle.space`}>
									{user.username}
								</a>
								's site
							</li>
						...
						{sites?.map((site) => {
							...
							return (
								<li key={site.hostname}>
									...
									<a href={`https://${site.username}.dawdle.space`}>
										{site.username}
									</a>
								</li>
							);
